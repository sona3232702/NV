...
# --- Expanded stub templates (to reduce repetition & add "game-like" prompts) ---
_EXTRA_TEMPLATES: Dict[str, List[Dict[str, Any]]] = {
    "VIS": [
        {"prompt_type":"COLOR","prompt_text":"What color is this?","payload":{"color_hex":"#F2D21B"}},  # yellow
        {"prompt_type":"COLOR","prompt_text":"What color is this?","payload":{"color_hex":"#2ECC71"}},  # green
        {"prompt_type":"COLOR","prompt_text":"What color is this?","payload":{"color_hex":"#3498DB"}},  # blue
        {"prompt_type":"COLOR","prompt_text":"What color is this?","payload":{"color_hex":"#E74C3C"}},  # red
        {"prompt_type":"COLOR","prompt_text":"What color is this?","payload":{"color_hex":"#9B59B6"}},  # purple
        {"prompt_type":"IMAGE","prompt_text":"What animal is this?","payload":{"emoji":"🦁","label":"lion"}},
        {"prompt_type":"IMAGE","prompt_text":"What animal is this?","payload":{"emoji":"🐶","label":"dog"}},
        {"prompt_type":"IMAGE","prompt_text":"What animal is this?","payload":{"emoji":"🐱","label":"cat"}},
        {"prompt_type":"IMAGE","prompt_text":"What animal is this?","payload":{"emoji":"🐸","label":"frog"}},
        {"prompt_type":"IMAGE","prompt_text":"What animal is this?","payload":{"emoji":"🐼","label":"panda"}},
    ],
    "EXEC": [
        {"prompt_type":"MATH","prompt_text":"What is 7 + 5?","payload":{"answer":12}},
        {"prompt_type":"MATH","prompt_text":"What is 9 - 4?","payload":{"answer":5}},
        {"prompt_type":"MATH","prompt_text":"What is 6 + 6?","payload":{"answer":12}},
        {"prompt_type":"MATH","prompt_text":"What is 8 - 3?","payload":{"answer":5}},
        {"prompt_type":"TEXT","prompt_text":"Count backwards from 20."},
        {"prompt_type":"TEXT","prompt_text":"Spell WORLD backwards."},
    ],
    "AUD": [
        {"prompt_type":"TEXT","prompt_text":"Repeat this word: 'hospital'."},
        {"prompt_type":"TEXT","prompt_text":"Repeat these numbers: 7-2-9."},
        {"prompt_type":"TEXT","prompt_text":"Repeat this word: 'banana'."},
        {"prompt_type":"TEXT","prompt_text":"Repeat these numbers: 4-1-8."},
    ],
    "LANG": [
        {"prompt_type":"TEXT","prompt_text":"Name three things you might find in a kitchen."},
        {"prompt_type":"TEXT","prompt_text":"Tell me what you did earlier today."},
        {"prompt_type":"TEXT","prompt_text":"What is your full name?"},
        {"prompt_type":"TEXT","prompt_text":"Name three animals."},
        {"prompt_type":"TEXT","prompt_text":"Say the days of the week."},
    ],
    "MOT": [
        {"prompt_type":"MOTOR","prompt_text":"Tap your fingers 5 times.","payload":{"reps":5}},
        {"prompt_type":"MOTOR","prompt_text":"Open and close your hand for 5 seconds.","payload":{"seconds":5}},
        {"prompt_type":"MOTOR","prompt_text":"Lift your right thumb.","payload":{}},
        {"prompt_type":"MOTOR","prompt_text":"Touch your nose, then your ear.","payload":{}},
        {"prompt_type":"MOTOR","prompt_text":"Hold your arms out straight.","payload":{"seconds":3}},
    ],
}
# Merge extras into base templates
for _k, _lst in _EXTRA_TEMPLATES.items():
    _STUB_TEMPLATES.setdefault(_k, [])
    _STUB_TEMPLATES[_k].extend(_lst)



def _anti_repeat(history: List[Dict[str, Any]], candidates: List[Dict[str, Any]], k: int = 10) -> List[Dict[str, Any]]:
    """Filter out recently-used templates by a simple fingerprint.
    Works whether `history` is prompt history or responses_log (must include prompt_text and may include payload).
    """
    recent = history[-k:] if len(history) >= k else history[:]
    recent_fp = set()
    for h in recent:
        pt = str(h.get("prompt_text", "")).strip().lower()
        pl = h.get("payload") or {}
        # keep payload small for fingerprint
        if isinstance(pl, dict):
            pl_items = tuple(sorted((str(a), str(b)) for a, b in list(pl.items())[:6]))
        else:
            pl_items = (str(pl),)
        recent_fp.add((pt, pl_items))

    def fp(c: Dict[str, Any]):
        pt = str(c.get("prompt_text", "")).strip().lower()
        pl = c.get("payload") or {}
        if isinstance(pl, dict):
            pl_items = tuple(sorted((str(a), str(b)) for a, b in list(pl.items())[:6]))
        else:
    ...
    filtered = [c for c in candidates if fp(c) not in recent_fp]
...
def _expected_lobes_for_domain(domain: str) -> List[str]:
    """Coarse mapping for Phase 2 lighting (judge-friendly baseline).
    Later, MedGemma can output parcel IDs and we can refine to parcels.
    """
    d = (domain or "").upper()
    if d == "LANG":
...
def next_prompt(profile: Dict[str, Any], history: List[Dict[str, Any]], sii_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    candidates = _anti_repeat(history, candidates, k=12)

    # Optional: avoid recently served prompts passed in sii_state
    try:
        avoid = set((sii_state or {}).get('avoid_prompt_hashes', []) or [])
        if avoid:
            def _fp_tmp(tmpl: dict) -> str:
                return str(tmpl.get('prompt_text','')).strip().lower()
            candidates2 = [c for c in candidates if _fp_tmp(c) not in avoid]
            if candidates2:
    ...
    return {
        ...
        "expected_lobes": _expected_lobes_for_domain(dom),
        "payload": payload,
